...
(function() {
    ...
    window.addEventListener('bymera-connect-wallet', async (event) => {

        const responseEvent = (data) => {
            window.dispatchEvent(new CustomEvent('bymera-wallet-response', { detail: data }));
        };
        ...
        try {
            ...
            window.__bymera_account = account;
    ...
    window.addEventListener('bymera-switch-network', async (event) => {

        const responseEvent = (data) => {
            window.dispatchEvent(new CustomEvent('bymera-network-response', { detail: data }));
        };
        ...
        try {
            ...
            if (currentChainId === chainId) {
                responseEvent({ success: true, message: 'Already on Base Sepolia network' });
                return;
            }
            ...
            try {
                ...
                responseEvent({ success: true, message: 'Switched to Base Sepolia network' });
            } catch (switchError) {
    ...
    window.addEventListener('bymera-switch-blockchain', async (event) => {

        const responseEvent = (data) => {
            window.dispatchEvent(new CustomEvent('bymera-blockchain-response', { detail: data }));
        };
    ...
    window.addEventListener('bymera-fetch-balances', async (event) => {

        const responseEvent = (data) => {
            window.dispatchEvent(new CustomEvent('bymera-balances-response', { detail: data }));
        };

        try {
            if (!window.ethereum || !window.__bymera_account) {
                responseEvent({
                    success: false,
            ...
            const account = window.__bymera_account;
            ...
            const tokenContracts = {
                ETH: null, // Native token, handled separately
            ...
            const tokenBalances = {
                ETH: '120.0', // Placeholder - ETH balance
                BNB: bnbAmount,
    ...
    window.addEventListener('bymera-fetch-bnb-price', async (event) => {

        const responseEvent = (data) => {
            window.dispatchEvent(new CustomEvent('bymera-price-response', { detail: data }));
        };
    ...
    window.addEventListener('bymera-send-transaction', async (event) => {

        const responseEvent = (data) => {
            window.dispatchEvent(new CustomEvent('bymera-transaction-response', { detail: data }));
        };
