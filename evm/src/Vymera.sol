...
contract Vymera is Ownable {
    ...
    function fund(
        ...
        emit Funded(_id, _fiatAmount, _currencyCode);
    }
    ...
    function refund(uint256 _id) external onlyOwner {
        ...
        emit Refunded(_id, funding.tokenAmount, funding.tokenAddress);
    }

    /// @notice Withdraw accumulated tokens (owner only)
    /// @param _tokenAddress Token address to withdraw
    function withdrawTokens(address _tokenAddress) external onlyOwner {
        SafeTransferLib.safeTransferAll(_tokenAddress, owner());
    }
